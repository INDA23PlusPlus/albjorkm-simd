// Taken from https://github.com/Hejsil/zig-bench
// Licensed under MIT
// Which can be found here: https://github.com/Hejsil/zig-bench/blob/master/LICENSE
// All copyrights go to Jimmi Holst Christensen (2023)
...
pub fn benchmark(comptime B: type) !void {
    ...
    inline for (functions) |def| {
        inline for (args, 0..) |arg, index| {
            var runtimes = try std.heap.page_allocator.alloc(u64, max_iterations);
            defer std.heap.page_allocator.free(runtimes);
